...
public class VisionAmbassadorRepositoryImpl implements CustomVisionAmbassador {
    ...
    public Page<VisionAmbassador> filterVisionAmbassador(String searchString, Pageable pageable) {
        ...
        AggregationResults<org.bson.Document> results = mongoTemplate.aggregate(aggregation, "vision_ambassadors",
        ...
        List<VisionAmbassador> visionAmbassadors = documents.stream()
                .map(doc -> {
                    ...
                    if (userDetails != null) {
                        // Decrypt each field after fetching
                        String firstName = encryptionService.decrypt(userDetails.getString("firstName"));
                        String lastName = encryptionService.decrypt(userDetails.getString("lastName"));
                        String fullName = (firstName != null ? firstName : "") + " "
                                + (lastName != null ? lastName : "");
                        va.setName(fullName.trim());
                        va.setUserId(userDetails.getObjectId("_id").toHexString());
                        va.setCity(encryptionService.decrypt(userDetails.getString("city")));
                        va.setState(encryptionService.decrypt(userDetails.getString("state")));
                        va.setPhoneNumber(encryptionService.decrypt(userDetails.getString("phoneNumber")));
                        va.setLanguage(encryptionService.decrypt(userDetails.getString("language")));
                    }
        ...
        AggregationResults<org.bson.Document> countResults = mongoTemplate.aggregate(countAggregation,
                "vision_ambassadors", org.bson.Document.class);
        long total = 0;
    ...
    // @Override
    // public Page<VisionAmbassadorResponse> filterVisionAmbassador(String state,
    // String city, String searchString, Pageable pageable) {
